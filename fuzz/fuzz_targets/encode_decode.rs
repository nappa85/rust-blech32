...
fn do_test(data: &[u8]) {
    ...
    let dp = data[hrp_end..]
        .iter()
        .map(|b| blech32::u5::try_from_u8(b % 32).unwrap())
        .collect::<Vec<_>>();
    ...
    if let Ok(data_str) = blech32::encode(&hrp, &dp, variant).map(|b32| b32.to_string()) {
        let decoded = blech32::decode(&data_str);
        let b32 = decoded.expect("should be able to decode own encoding");

        assert_eq!(blech32::encode(&b32.0, &b32.1, b32.2).unwrap(), data_str);
    }
}
